...
def test_candidate_game_to_dict() -> None:
    ...
    assert sorted(list(game_dict.keys())) == sorted(["anchor_player_id", "team_x", "team_y", "imbalance"])
...
def test_when_providing_queue_weight_then_priority_is_computed() -> None:
    ...
    game_dict = candidate_game.to_dict()
    assert "priority" in game_dict.keys()
    assert "imbalance" not in game_dict.keys()
